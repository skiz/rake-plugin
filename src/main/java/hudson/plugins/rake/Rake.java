...
public class Rake extends Builder {
  ...
  private boolean hasGemfile(FilePath dir) {
    try {
      return new FilePath(dir, "Gemfile").exists();
    } catch (Exception e) {
      return false;
    }
  }
  ...
  @Override
    public boolean perform(AbstractBuild<?,?> build, Launcher launcher, BuildListener listener) throws InterruptedException {
      ...
      if (rake != null) {
        ...
        if(hasGemfile(workingDir)) {
          args.add("bundle", "exec", exec.getPath());
        } else {
          args.add(exec.getPath());
        }
      } else {
        if(lastBuiltLauncher.isUnix()) {
          if(hasGemfile(workingDir)) {
            args.add("bundle", "exec", "rake");
          }else{
            args.add("rake");
          }
        } else {
